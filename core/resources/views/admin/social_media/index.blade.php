                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-end flex-wrap gap-1">
                                                <button class="btn btn-outline--primary editBtn cuModalBtn btn-sm" data-modal_title="@lang('Update Platform / Type')" data-resource="{{ $socialMedia }}">
                                                    <i class="las la-pen"></i>@lang('Edit')
                                                </button>
                                                <a class="btn btn-sm btn-outline--info" href="{{ route('admin.social.media.info', $socialMedia->id) }}">
                                                    <i class="la la-info-circle"></i> @lang('Field Setup')
                                                </a>

                                                @if ($socialMedia->status == Status::ENABLE)
                                                    <button class="btn btn-outline--danger btn-sm confirmationBtn" data-question="@lang('Are you sure to disable this platform?')" data-action="{{ route('admin.social.media.status', $socialMedia->id) }}">
                                                        <i class="las la-eye-slash"></i>@lang('Disable')
                                                    </button>
                                                @else
                                                    <button class="btn btn-outline--success confirmationBtn btn-sm" data-question="@lang('Are you sure to enable this platform?')" data-action="{{ route('admin.social.media.status', $socialMedia->id) }}">
...
@push('breadcrumb-plugins')
    <x-search-form />
    <button class="btn btn-sm btn-outline--primary cuModalBtn" data-modal_title="@lang('Add Service Platform / Type')">
